...
class ModelBooking extends CI_Model { 
    ...
    public function joinOrder($where) { 
        ...
        $this->db->join('fasilitas bu ', 'bu.id=d.id_fasilitas'); 
        $this->db->where($where); 
        return $this->db->get(); 
    } 
    
    public function simpanDetail($where = null) { 
        $sql = "INSERT INTO booking_detail (id_booking,id_fasilitas) SELECT booking.id_booking,temp.id_fasilitas FROM booking, temp WHERE temp.id_user=booking.id_user AND booking.id_user='$where'"; 
        $this->db->query($sql); 
    } 
    ...
    public function find($where) { 
        ...
        $this->db->limit(1); 
        return $this->db->get('fasilitas', $where); 
    } 
    ...
    public function createTemp() { 
        $this->db->query('CREATE TABLE IF NOT EXISTS temp(id_booking varchar(12), tgl_booking DATETIME, email_user varchar(128), id_fasilitas int)'); 
    } 
    ...
    public function selectJoin() { 
        ...
        $this->db->join('fasilitas', 'booking_detail.id_fasilitas=fasilitas.id'); 
        return $this->db->get(); 
    }
    ...
    public function hapusTransaksi($where = null) {
        $this->db->delete('detail_pakai', $where);
    }
}
